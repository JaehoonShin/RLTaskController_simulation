...
COLUMNS = ['rpe', 'spe', 'mf_rel', 'mb_rel', 'p_mb', 'd_p_mb', 'ctrl_reward', 'score'] + ACTION_COLUMN + ['applied_reward']
PCA_COMPONENTS = 2
...
#        self.file_name  = (lambda x: RESULTS_FOLDER + x + ' ' + datetime_str)
        self.file_name = (lambda x: RESULTS_FOLDER + x)

    def complete_simulation(self):
        self.current_data.append(self.current_df)
        self.current_detail.append(self.current_detail_df)

    def fast_initialization_DATA(self, tot_len_data):
        self.current_df = pd.DataFrame(index= range(tot_len_data), columns=COLUMNS)

    def fast_initialization_DETAIL(self, tot_len_detail):
        self.current_detail_df = pd.DataFrame(index= range(tot_len_detail), columns=DETAIL_COLUMNS)
    ...
    def plot_opposite_nn_compare_learning_curve(self, mode, subject_id):
        data_df1 = self.data[MODE_IDENTIFIER][0]
        detail_df1 = self.detail[MODE_IDENTIFIER][0]
        data_df2 = self.data[MODE_IDENTIFIER2][0]
        detail_df2 = self.detail[MODE_IDENTIFIER2][0]
        data_df3 = self.data[MODE_IDENTIFIER3][0]
        detail_df3 = self.detail[MODE_IDENTIFIER3][0]       
        target_val = MODE_MAP[mode][0]
        episode_index1 = data_df1['ctrl_reward'].loc[0.2 * len(data_df1):].idxmax() #.rolling(EPISODE_SMOOTH_WINDOW).mean().idxmax()
        episode_index2 = data_df2['ctrl_reward'].loc[0.2 * len(data_df2):].idxmax()
        episode_index3 = data_df3['ctrl_reward'].loc[0.2 * len(data_df3):].idxmax()
        original_series = detail_df3[target_val].loc[episode_index3 * self.trial_separation :
                                       (episode_index3 + 1) * self.trial_separation - 1].copy().tolist()
        opposite_series = detail_df2[target_val].loc[episode_index2 * self.trial_separation :
                                       (episode_index2 + 1) * self.trial_separation - 1].copy().tolist()
        new_series = detail_df1[target_val].loc[episode_index1 * self.trial_separation :
                                       (episode_index1 + 1) * self.trial_separation - 1].copy().tolist()
        if target_val == 'spe':
            plt.plot(original_series, label='Optimal Sequence', color=MODE_MAP[mode][2])
            plt.plot(opposite_series, label='Opposite Sequence', color='black')
            plt.plot(new_series, label = 'NN Squence', color = MODE_MAP[MODE_IDENTIFIER][2])
            plt.title(mode + ', compare 3 sequence of subject ' + str(subject_id) + ' transferred SPE curve')
        else: # traget_val == 'rpe'
            plt.plot(original_series, label='Optimal Sequence', color=MODE_MAP[mode][2])
            plt.plot(opposite_series, label='Opposite Sequence', color='black')
            plt.plot(new_series, label = 'NN Squence', color = MODE_MAP[MODE_IDENTIFIER][2])
            plt.title(mode + ', compare 3 sequence of subject ' + str(subject_id) + ' transferred RPE curve')
        plt.legend(loc='best')
        save_plt_figure(self.file_name(mode + ', compare 3 sequence of subject ' + str(subject_id) + ' transferred curve'))
        ...
        if target_val !=None:
            ...
            plt.title(mode + ', compare 3 sequence of subject ' + str(subject_id) + ' transferred RPE curve')
            plt.legend(loc='best')
            save_plt_figure(self.file_name(mode + ', compare 3 sequence of subject ' + str(subject_id) + ' transferred curve 2'))
    ...
    def _plot_learning_curve(self, title): # plot smooth learning curve
        ...
        for index, (data_df, detail_df) in enumerate(zip(self.current_data, self.current_detail)):
            ...
            if CONFIDENCE_INTERVAL:
                standard_error = ctrl_reward_series.rolling(smooth_val).apply(lambda a: sem(a))
                ...
                mmin = ctrl_reward_series.rolling(smooth_val).apply(lambda a: np.quantile(a, 0.25))
                mmax = ctrl_reward_series.rolling(smooth_val).apply(lambda a: np.quantile(a, 0.75))
    ...
    def _plot_p_mb2(self, mode_lst, subject_lst, subject_info, mode):
        ...
        for index in range(len(mode_lst)):
            ...
            discrete_prob_df = pd.DataFrame(columns=TICK_NAME_STR)
            interval_len = 1 / self.trial_separation
            for sub_index in range(EPISODE_SMOOTH_WINDOW//2):
                discrete_list = np.zeros(self.trial_separation)
    ...
    ############## personally added at 20200409 #############
    def get_optimal_control_index(self, mode, subject_id):
        data_df_lst = self.data[mode]
        data_df = data_df_lst[subject_id]
        detail_df_lst = self.detail[mode]
        detail_df = detail_df_lst[subject_id]
