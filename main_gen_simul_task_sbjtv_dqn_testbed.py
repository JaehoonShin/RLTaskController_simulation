...
if __name__ == '__main__':
    ...
    # make folder to save
    SIMUL_BEHAV_DIR = 'simul_results/T' + str(TASK_IDF) + '_more/'
    try:
        os.mkdir(SIMUL_BEHAV_DIR)
    except:
        ''
                    ...
                    if MODEL_LIST_IDF == 3:
                        ...
                        if simul_no_fit == True:
                            ...
                            sim.simulation(*(param_list[index]), PARAMETER_SET=str(index), bhv_pseudo = bhv_pseudo,
                                           ...
                                           NLL_sbj=NLL_list[index], no_fitting = simul_no_fit, player_MDP = player_MDP)
                        elif bhv_pseudo == False:
                            sim.simulation(*(param_list[index]), PARAMETER_SET=str(index), bhv_pseudo=bhv_pseudo, TRIALS_PER_EPISODE_sbjtv = sim.TRIALS_PER_EPISODE_sbjtv, NLL_sbj=NLL_list[index])
                        else:
                            sim.simulation(*(param_list[index]), PARAMETER_SET=str(index), bhv_pseudo=bhv_pseudo,
                                           TRIALS_PER_EPISODE_sbjtv=sim.TRIALS_PER_EPISODE_sbjtv)
