...
import csv
import os
import simulation_A2C_reader as sim
import analysis
        ...
        if ALL_MODE:
            for mode, _ in MODE_MAP.items():
                ...
                if sim.mode202010 == False:
                    print('Parameter set: ' + str(param_list[0]))
                if sim.mode202010:
                    ...
                    sim.simulation(*(param_list), PARAMETER_SET=str(index))
#                gData.generate_summary(sim.CONTROL_MODE)
            ...
            print('Parameter set: ' + str(param_list[0]))
            sim.simulation(*(param_list), PARAMETER_SET=str(0))
