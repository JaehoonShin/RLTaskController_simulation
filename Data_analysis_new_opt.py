...
if __name__ == '__main__':
    short_opt = "hdn:"
    long_opt  = ["sbj=","mode=","task=","file-suffix=","folderpath="]
    try:
...
opt_index= data.data[pol_list[pol]][0]['ctrl_reward'].loc[ceil(0.2 * len(data.data[pol_list[pol]][0])):].sort_values().tail(100).index
...
plt.title(control_mode + ' ' + str(sbj_indx) + ' top 100 pols')
plt.savefig('history_results/' + folderpath + '/'+control_mode + ' ' + str(sbj_indx) + ' top 100 pols_plot' + file_suffix + '.png')
plt.clf()
